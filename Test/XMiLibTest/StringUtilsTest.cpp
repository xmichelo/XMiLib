...
void XMiLibTest::stringUtilsToAndFromJsonDoc() {
    try {
        for (qint32 i = 0; i < 1000; ++i) {
            ...
            QStringList result;
            QString errMsg;
            QVERIFY2(jsonDocumentToStringList(doc, result, &errMsg), QString("jsonDocumentToStringList() returned false. Error message = %1").arg(errMsg).toLocal8Bit());
            QVERIFY2(result == source, "Converting back and forth returned a result that is not the source.");
        }
        QJsonDocument const doc;
        QStringList strList;
        QVERIFY2(!jsonDocumentToStringList(doc, strList), "An invalid document does not result in an error by jsonDocumentToStringList().");
        QString errMsg;
        QVERIFY2(!jsonDocumentToStringList(doc, strList, &errMsg), "An invalid document does not result in an error by jsonDocumentToStringList().");
...
void XMiLibTest::stringUtilsToAndFromJsonFile() {
    try {
        for (qint32 i = 0; i < 20; ++i) {
            ...
            QString errMsg;
            QVERIFY2(saveStringListToJsonFile(source, path, &errMsg), QString("saveStringListToJsonFile() returned false. Error message = %1").arg(errMsg).toLocal8Bit());
            QStringList result;
            QVERIFY2(loadStringListFromJsonFile(path, result, &errMsg), QString("loadStringListFromJsonFile() returned false. Error message = %1").arg(errMsg).toLocal8Bit());
