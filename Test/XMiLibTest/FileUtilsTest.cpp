...
// 
//****************************************************************************************************************************************************
void XMiLibTest::fileUtilsGetRandomFileName() {
    try {
        ...
        for (qint32 i = 0; i < 100; ++i) {
            ...
            QVERIFY2(fileName.length() == 1 + extLength + baseLength + prefixSize, "The generated file name does not have the appropriate length.");
            ...
            QVERIFY2(baseName.length() == baseLength + prefixSize, "The generated file name does not have the expected base name.");
            QVERIFY2(QRegularExpression(QString(R"(^.{%1}[0-9a-z]+$)").arg(prefixSize)).match(baseName).hasMatch(), "The generated base name is not valid.");
            if (usePrefix)
...
void XMiLibTest::fileUtilsGetTempFilePath() {
    try {
        ...
        for (qint32 i = 0; i < 1000; ++i) {
            ...
            QVERIFY2(tempPath == QDir(fileInfo.absolutePath()).canonicalPath(), "The temporary file is not in the temporary folder.");
            ...
            if (extLen)
                QVERIFY2(fileName.endsWith(QString(".%1").arg(ext)), "The temporary file does not have the required extension");
        }
    }
    catch (...) {
        QVERIFY2(false, "The function threw an exception.");
...
// 
//****************************************************************************************************************************************************
void XMiLibTest::fileUtilsCreateTempFile() {
    try {
